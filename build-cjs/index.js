...
// Validation schemas
const VmNameSchema = zod_1.z.object({
    vm: zod_1.z.string().min(1, 'VM name cannot be empty'),
});
const StopVmSchema = zod_1.z.object({
    vm: zod_1.z.string().min(1, 'VM name cannot be empty'),
    force: zod_1.z.boolean().optional(),
});
const ExecVmCommandSchema = zod_1.z.object({
    vm: zod_1.z.string().min(1, 'VM name cannot be empty'),
...
function executePrlctl(command, ...args) {
    try {
        ...
        const result = (0, child_process_1.execSync)(fullCommand, {
            encoding: 'utf-8',
